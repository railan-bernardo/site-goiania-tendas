...
class Web extends Controller
{
    ...
    public function contact(): void
    {
        $siteConfig =  site()->findById(1);
        $head = $this->seo->render(
            "Contato | " . $siteConfig->title . " - " . $siteConfig->description,
            $siteConfig->description,
            url("/contato"),
             theme("/assets/images/share.png")
        );
 ...
 /**
    ...
    public function allCategory(): void
    {
        ...
        $siteConfig =  site()->findById(1);
        $head = $this->seo->render(
            "Categorias " . $siteConfig->title . " - " . $siteConfig->description,
            $siteConfig->description,
            url("/services"),
             theme("/assets/images/share.png")
        );
         ...
         $service = (new ServiceCategory())->find();
       
        $pager = new Pager(url("/service/s/"));
        $pager->pager($service->count(), 9, ($data['page'] ?? 1));

        echo $this->view->render("service", [
            "head" => $head,
            "subcategories" =>$subcategory->fetch(true),
            "services" =>$service->limit($pager->limit())->offset($pager->offset())->fetch(true),
            "paginator"=>$pager->render()
        ]);
    }
    ...
    public function productDetail(array $data): void
    {
        ...
        echo $this->view->render("product-details", [
            ...
            ->find("id_service = :id_service", "id_service={$post->id}")
            ->fetch(true),
            "sizes"=> (new PostSize())
