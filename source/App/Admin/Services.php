...
class Services extends Admin
{
    ...
    public function post(?array $data): void
    {
        ...
        if (!empty($data["action"]) && $data["action"] == "create") {
            ...
            $postCreate->specification = $data["specification"];
            $postCreate->structure = $data["structure"];
            ...
            $postCreate->post_at = date_fmt_back($data["post_at"]);
             ...
             if (!empty($_FILES["icon"])) {
                ...
                $files = $_FILES["icon"];
                $upload = new Upload();
                $icon = $upload->image($files, $postCreate->title . "-icon");
            ...
            $json["redirect"] = url("/admin/produto/post_size/{$postCreate->id}");
        ...
        if (!empty($data["action"]) && $data["action"] == "update") {
            ...
            $postEdit->specification = $data["specification"];
            $postEdit->structure = $data["structure"];
            ...
            // upload icon
            if (!empty($_FILES["icon"])) {
                if ($postEdit->icon && file_exists(__DIR__ . "/../../../" . CONF_UPLOAD_DIR . "/{$postEdit->icon}")) {
                    unlink(__DIR__ . "/../../../" . CONF_UPLOAD_DIR . "/{$postEdit->icon}");
                   
                }

                $files = $_FILES["icon"];
                $upload = new Upload();
                $icon = $upload->image($files, $postEdit->title . "-icon");
        ...
        if (!empty($data["action"]) && $data["action"] == "delete") {
            ...
            if (!$postDelete) {
              ...
              if ($postDelete->icon && file_exists(__DIR__ . "/../../../" . CONF_UPLOAD_DIR . "/{$postDelete->icon}")) {
                unlink(__DIR__ . "/../../../" . CONF_UPLOAD_DIR . "/{$postDelete->icon}");
                  //unlink(__DIR__ . "/../../../" . CONF_UPLOAD_DIR . "/{$postDelete->video}");
                (new Thumb())->flush($postDelete->icon);
            }
            ...
            $tm = (new PostSize())->find()->fetch(true);

            foreach ($tm as $value) {
               
               if($postDelete->id == $value->idpost){
   
                        unlink(__DIR__ . "/../../../" . CONF_UPLOAD_DIR . "/{$value->cover}");
                        (new Thumb())->flush($value->cover);
            ...
            $color = (new Colors())->find()->fetch(true);

            foreach ($color as $value) {
               
               if($postDelete->id == $value->id_product){
